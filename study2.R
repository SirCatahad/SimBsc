...
for(i in 1:iter)
{
  ...
  for(cr in zip(covariances, r.squared))
  {
    ...
    data <- try(with(parameters, simData2(n    = n,
                                         coefficients = c[[1]],
                                         covariance   = cr[[1]],
                                         r.sqrd       = cr[[2]])))
    ...
    reg <- lm(Y ~ X + I(X^2), data=data[complete.cases(data),])
    
    ## Maybe we'll need to change 
    storage[con, ] <- c(summary(reg)$coefficients[c(1,3)],
                        summary(reg)$coefficients[c(4,6)],
                        confint(reg)[c(1,3),1],
                        confint(reg)[c(1,3),2],
                        con)
    ...
    #run regression model
    reg <- lm(Y ~ X + I(X^2), data=data_MCAR[complete.cases(data_MCAR),])
    ...
    for(s in snr)
    {
      ...
      reg <- lm(Y ~ X + I(X^2), data=data_MAR[complete.cases(data_MAR),])
    ...
    for(d in dList)
    {
      ...
      #Store
      reg <- lm(Y ~ X + I(X^2), data=pdmi_Data[complete.cases(pdmi_Data),])
    ...
    for(match in mtype)
    {
      for(k in km)
      {
        for(d in dList)
        {
          ...
          reg <- lm(Y ~ X + I(X^2), data=pmm_Data[complete.cases(pmm_Data),])
